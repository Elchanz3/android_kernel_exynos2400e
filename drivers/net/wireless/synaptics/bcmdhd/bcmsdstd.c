...
sdioh_attach(osl_t *osh, void *bar0, uint irq)
{
	...
	sd->mem_space = (volatile char *)sdstd_reg_map(osh, *((dmaaddr_t *)(&bar0)), SDIOH_REG_WINSZ);
	sd_init_dma(sd);
	sd->irq = irq;
		...
		if (GFIELD(cmd_arg, CMD53_BLK_MODE)) {
			...
			if (use_dma) {
				...
				case DMA_MODE_SDMA:
					...
					sdstd_wreg(sdioh_info, SD_SysAddr, *((uint32 *)(&sdioh_info->dma_phys)));
					break;
				case DMA_MODE_ADMA1:
				case DMA_MODE_ADMA2:
...
#endif /* BCMSDIOH_TXGLOM */
					{
						...
						*((uint32 *)(&sdioh_info->dma_phys)), blockcount*blocksize,
...
sd_map_dma(sdioh_info_t * sd)
{
	...
	if ((va = DMA_ALLOC_CONSISTENT(sd->osh, dma_buf_size, SD_PAGE_BITS, &alloced,
		...
		// change for compatibility for 'sd->dma_phys = ROUNDUP((sd->dma_start_phys), SD_PAGE);'
		{
#ifdef BCMDMA64OSL
	...
	if ((va = DMA_ALLOC_CONSISTENT(sd->osh, SD_PAGE, SD_PAGE_BITS, &alloced,
		...
		// change for compatibility for 'sd->adma2_dscr_phys = ROUNDUP((sd->adma2_dscr_start_phys), SD_PAGE);'
		{
#ifdef BCMDMA64OSL
