...
static bool dhdpcie_is_rom_protect_set(dhd_bus_t *bus);
#if defined(FW_SIGNATURE)
static bool dhdpcie_is_vector_protect_set(dhd_bus_t *bus);
...
static int dhdpcie_read_fwstatus(dhd_bus_t *bus, bcm_fwsign_verif_status_t *status);
#endif /* FW_SIGNATURE */
	...
	if (enter) {
		...
		if (si_setcore(bus->sih, ARMCA7_CORE_ID, 0)) {
...
#if defined(FW_SIGNATURE)
			secure_mode = dhdpcie_is_vector_protect_set(bus);
			if (secure_mode == TRUE || bus->fwsig_filename[0] != 0) {
				DHD_ERROR(("%s: Secure Boot %s, Signature File %s "
						"Entering Secure Boot Mode \n",
						__FUNCTION__, ((secure_mode == TRUE)?
						"Enabled":"Disabled"),
						bus->fwsig_filename));
				if ((bcmerror =
			...
			if (do_wr_flops) {
				...
				if (!dhdpcie_is_rom_protect_set(bus)) {
			...
			if (do_wr_flops) {
				...
				if (!dhdpcie_is_rom_protect_set(bus) && bcmerror == BCME_OK) {
					uint32 tmp;
...
static int
dhdpcie_read_fwstatus(dhd_bus_t *bus, bcm_fwsign_verif_status_t *status)
{
	int ret = BCME_OK;
...
dhd_bus_handle_mb_data(dhd_bus_t *bus, uint32 d2h_mb_data)
{
	...
	if (d2h_mb_data & D2H_DEV_DS_EXIT_NOTE)  {
		...
		if (INBAND_DW_ENAB(bus)) {
			 ...
			 */
			/* Also bypass sleep if DHD is busy, for example, transmitting frames */
			if (bus->deep_sleep && DHD_BUS_BUSY_CHECK_IDLE(bus->dhd)) {
				dhd_bus_set_device_wake(bus, FALSE);
...
#if defined(FW_SIGNATURE)
	if (dhdpcie_is_vector_protect_set(bus) == TRUE) {
		bcm_fwsign_verif_status_t status;

		if (dhdpcie_read_fwstatus(bus, &status) == BCME_OK) {
