...
#else
#define MACINFO_EFS "/mnt/vendor/efs/wifi/.mac.info"
#endif /* PLATFORM_SLP */
#else
...
dhd_create_random_mac(char *buf, unsigned int buf_len)
{
	...
	snprintf(buf, buf_len, MAC_CUSTOM_FORMAT, (uint32)0x00, (uint32)0x12, (uint32)0x34,
...
int
dhd_read_macaddr_from_file(dhd_pub_t *dhdp, struct ether_addr *dest_mac)
{
...
#endif /* DHD_MAC_ADDR_EXPORT_RO */
	}

#ifdef DHD_MAC_ADDR_EXPORT_RO
	/* Write MAC address to /sys/ file */
	if (!bcm_ether_atoe(mac_buf, &sysfs_mac_addr)) {
		DHD_ERROR(("%s : mac parsing err\n", __FUNCTION__));
		return BCME_ERROR;
	} else {
		DHD_ERROR(("%s: MAC address [%s] written into pseudo filen",
...
dhd_set_macaddr_from_file(dhd_pub_t *dhdp)
{
	...
	dhd_read_macaddr_from_file(dhdp, mac);
...
#endif /* (USE_CID_CHECK || USE_DIRECT_VID_TAG) && DHD_EXPORT_CNTL_FILE */
			/* update MAC address */
