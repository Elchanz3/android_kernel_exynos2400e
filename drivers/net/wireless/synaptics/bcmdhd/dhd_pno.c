...
dhd_pno_process_epno_result(dhd_pub_t *dhd, const void *data, uint32 event, int *size)
{
	...
	*size = 0;
	...
	if (event == WLC_E_PFN_NET_FOUND || event == WLC_E_PFN_NET_LOST) {
		const wl_pfn_scanresults_v1_t *pfn_result = (const wl_pfn_scanresults_v1_t *)data;
		const wl_pfn_scanresults_v2_t *pfn_result_v2 = (const wl_pfn_scanresults_v2_t *)data;
		const wl_pfn_scanresults_v3_t *pfn_result_v3 = (const wl_pfn_scanresults_v3_t *)data;
	...
	*buf_len = 0;
	if (results_v1->version == PFN_SCANRESULTS_VERSION_V1) {
		...
		*buf_len = malloc_size;
		...
		const wl_pfn_net_info_v2_t *pnetinfo_v2 = (const wl_pfn_net_info_v2_t*)&results_v2->netinfo[0];
