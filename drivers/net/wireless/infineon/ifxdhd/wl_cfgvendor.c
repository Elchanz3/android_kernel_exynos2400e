		...
		struct wireless_dev *wdev, const void *data, int len)
{
	...
	memset(&haldata, 0x00, sizeof(struct buf_data));
	buf = &haldata;
	nla_for_each_attr(iter, data, len, rem) {
		type = nla_type(iter);
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, 0)) || defined(CONFIG_SOC_S5E5515)
...
#endif /* LINUX_VERSION_CODE < KERNEL_VERSION(4, 13, 0) || defined(CONFIG_SOC_S5E5515) */
