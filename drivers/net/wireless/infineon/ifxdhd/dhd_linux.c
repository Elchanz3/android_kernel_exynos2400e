...
#ifdef DHD_WAKEUP_PKT_DUMP
volatile bool dhd_is_mmc_wakeup = FALSE;
#endif /* DHD_WAKEUP_PKT_DUMP */
...
#ifdef DHD_WAKEUP_PKT_DUMP
		if (dhd_is_mmc_wakeup) {
			DHD_ERROR(("wake_pkt] protocol: 0x%-4x\n",
				ntoh16(skb->protocol)));
			dhd_prhex("wake_pkt", eth, MIN(len, 0x30), DHD_ERROR_VAL);
			dhd_is_mmc_wakeup = FALSE;
		}
#endif /* DHD_WAKEUP_PKT_DUMP */
...
bool dhd_update_fw_nv_path(dhd_info_t *dhdinfo)
{
	...
	if (!dhd_download_fw_on_driverload) {
#ifdef CONFIG_IFXDHD_FW_PATH
		fw = VENDOR_PATH CONFIG_IFXDHD_FW_PATH;
#endif /* CONFIG_IFXDHD_FW_PATH */
#ifdef CONFIG_IFXDHD_NVRAM_PATH
		nv = VENDOR_PATH CONFIG_IFXDHD_NVRAM_PATH;
#endif /* CONFIG_IFXDHD_NVRAM_PATH */
	}

#ifdef DHD_SSW_SPECIFIC_REQ
	if (firmware_path[0] != '/') {
		sprintf(temp_fw, VENDOR_FIRMWARE_PATH"/%s", firmware_path);
	}
	if (nvram_path[0] != '/') {
		sprintf(temp_nv, VENDOR_FIRMWARE_PATH"/%s", nvram_path);
	}
#endif // endif
...
#define EVENT_LOG_RATE_HC_THRESHOLD	1000
	...
	wl_pm2_sleep_ret_ext_t pm2sleep;
...
#ifdef DHD_SSW_P2P_USE_PERM_ADDRESS
	/* store perm_addr from current primary mac address */
	if (!ETHER_ISNULLADDR(dhd->mac.octet)) {
		memcpy(dhd_linux_get_primary_netdev(dhd)->perm_addr,
			dhd->mac.octet, ETHER_ADDR_LEN);
	}
#endif /* DHD_SSW_P2P_USE_PERM_ADDRESS */
	...
	memset(&pm2sleep, 0x0, sizeof(wl_pm2_sleep_ret_ext_t));

	pm2sleep.logic = 1;
	pm2sleep.low_ms= 50;
	pm2sleep.high_ms = 200;
	pm2sleep.rx_pkts_threshold = 5;
	pm2sleep.tx_pkts_threshold = 5;

	ret = dhd_iovar(dhd, 0, "pm2_sleep_ret_ext", (char *)&pm2sleep, sizeof(wl_pm2_sleep_ret_ext_t),
					NULL, 0, TRUE);
	if (ret < 0)
		DHD_ERROR(("%s pm2_sleep_ret_ext ret= %d\n", __FUNCTION__, ret));
...
dhd_set_blob_support(dhd_pub_t *dhdp, char *fw_path)
{
	struct file *fp;
#ifdef DHD_SSW_SPECIFIC_REQ
	char *filepath = CONFIG_IFXDHD_CLM_PATH;
#else
	char *filepath = VENDOR_PATH CONFIG_IFXDHD_CLM_PATH;
#endif /* DHD_SSW_SPECIFIC_REQ */
